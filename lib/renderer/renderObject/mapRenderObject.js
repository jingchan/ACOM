...
function MapRenderObject(renderer, map){
  ...
  this.tileMap.create('tileLayer', map.width, map.height, tileSize, tileSize, this.display);

  // reminder: visibilityMask is transposed; so do
  // visibilityMask[y][x].
  this.visibilityMask = [];
...
exports.prototype._addMapToStage = function (){
  ...
  map.eachTile(function(tile, i, j){
    ...
    if (!that.visibilityMask[j][i]) {
      tileMap.getTile(i,j).alpha = 0.5;
    } else {
      tileMap.getTile(i,j).alpha = 1;
    }
  });
};

exports.prototype.applyVisibilityMask = function (visibleTiles) {
  this.visibilityMask = visibleTiles;
  this.dirty = true;
};