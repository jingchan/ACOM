...
exports.prototype.render = function() {
  ...
  if (this.state === 'hiding') {
    ...
    this.selectionDisplay.position.x = this.position.x + 0.5*this.tileSize;
    for (var i = 0; i < this.menuTexts.length; i++) {
      var gap = i*this.tileSize;
      this.menuTexts[i].position.x = this.position.x + this.tileSize;
      this.menuTexts[i].position.y = this.position.y + this.tileSize*0.5 + gap;
    }
...
exports.prototype._generateHoverMenu = function(){
  ...
  this.selectionDisplay = game.add.text(0.5*tileSize, 0.5*tileSize, '>', this.fontStyle);

  // generate all texts
  this.menuTexts = [];
  for (var i = 0; i < menu.items.length; i++) {
    var gap = i*tileSize;
    this.menuTexts.push(game.add.text(tileSize, 0.5*tileSize + gap, menu.items[i].text,
                                      this.fontStyle));
  }
};

exports.prototype._repositionHoverMenu = function(){
  this.position.x = Math.max(this.menu.tilePosition.x * this.tileSize, 0);
  this.position.y = Math.max(this.menu.tilePosition.y * this.tileSize, 0);

	// Flip to left of cursor if horizontal overflow
	if(this.position.x > this.game.width - this.menuSize.x){
		this.position.x -= this.menuSize.x - this.tileSize;
	}
