...
/*
 * handles agent's (TODO: team's) range of visible tiles
 *
 * I guess eventually should be delegated to the renderObject?
 */
exports.prototype.handleVisibleRange = function(actor) {
  var visibleTiles = this.scene.map.visibleTiles(
    actor.owner.position.x,
    actor.owner.position.y,
    actor.sightRange);
  var mapRO = this.scene.map.renderObject;
  mapRO.handleVisibleTiles(visibleTiles);
};