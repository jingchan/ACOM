  ...
  renderer._tileToPosition = function(x, y) {
	...
	scene.on('entityAdded', function(entity){
      ...
      entity.on('successMove', function(data) {
        Debug.log("Renderer working", entity.name);
        ...
        Debug.log("Sanity", [source, target, dist]);
        visual.shouldRender = true;
      });
...
exports.prototype.renderScene = function(){
  ...
  //       entities on an event-based manner and not in a loop. My goal is to
  //       get rid of shouldRender
  //
	// if(scene.entities){
