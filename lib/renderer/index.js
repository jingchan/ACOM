...
var SensoryRenderObject = require('./renderObject/sensoryComponentRenderObject');
var RangeRenderObject = require('../renderer/renderObject/rangeRenderObject');
...
function CreateRenderer(app, scene, viewx, viewy, tileSize, gameDiv, completionHandler){
  ...
  // TODO: Components added/removed from existing entity are not properly handled
    ...
    //var sensoryRenderObject = new SensoryRenderObject(that, map);
    //that.addRenderObject(sensoryRenderObject, that.fowLayer);
    //map.sensoryRenderObject = sensoryRenderObject;
	});
	...
	scene.on('mapWillBeRemoved', function(map){
      ...
      //that.removeRenderObject(map.sensoryRenderObject);
      //map.sensoryRenderObject = null;
    }
	});
...
/**
 * Gives a uniform way of adding and removing renderObjects.
 * - Supply the layer to put the renderObject.
 * - in the constructor for the renderObject, put the display
 *   elements in renderObject.display;
 * - by "layer", I think we mean "Group"
 */
...
exports.prototype.addEntity = function(entity){
  ...
  if (visual) {
    ...
    if (moveComp) {
      var rangeRO = new RangeRenderObject(this, this.scene.map, moveComp.movabilityRange);
      this.addRenderObject(rangeRO, this.fowLayer);
  ...
  var sensoryComp = entity.getComponent('sensory');
  if(sensoryComp){
    var sensoryRenderObject = new SensoryRenderObject(this, sensoryComp);
    this.addRenderObject(sensoryRenderObject, this.fowLayer);
  }
