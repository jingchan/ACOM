...
function CreateRenderer(scene, game){
  ...
  renderer.game = game;
	...
	// renderer.viewport = [20 * renderer.tileSize, 17 * renderer.tileSize];
...
exports.prototype.renderScene = function(){
	...
	if(scene.entities){
		scene.entities.forEach(function(entity){
			// Update positions
			entity.sprite.reset((entity.position.x + 0.5) * that.tileSize,
			                    (entity.position.y + 0.5) * that.tileSize);
 ...
 * @private
 */
function _addMapToStage(map, game){
	if(!map || !map.grid){
		return;
	}
  var that = this;
