...
function CreateRenderer(app, scene, viewx, viewy, tileSize, gameDiv, completionHandler){
  ...
  // TODO: Components added/removed from existing entity are not properly handled
    ...
    that.addRenderObject(mapRenderObject, that.game.mapLayer);
...
exports.prototype.addEntity = function(entity){
  ...
  entity.display = this.game.add.group(this.game.entityLayer);
  ...
  if (healthComp) {
    ...
    // var coord = that._screenCoordFromGridCoord(visual.owner.position.x, visual.owner.position.y);
    // visual.healthText = game.add.text(coord.x, coord.y, healthComp.health.toString(),
