...
/**
 * Returns movable tiles centered at (x,y) with range {range}.
 * format: a grid, so accessing point (x,y) is visibleGrid[y][x].
 *   X_X
 *
 * TODO: flying, etc.
 */
exports.prototype.movableTiles = function(x, y, range) {
  //first find a rectangle with the L1 norm of the range
  ...
  for (var j = 0; j < this.height; j++) {
   	movableGrid[j] = [];
   	for (var i = 0; i < this.width; i++) {
   		movableGrid[j][i] = ((Math.abs(i-x) + Math.abs(j-y)) <= range);
   	}
  }
