...
exports.prototype.makeMenu = function(renderer) {
  ...
  for (var i = 0; i < self.actions.length; i++) {
    ...
    var returnFunc = function(actionName, eventName, actionData){
      return function(){
        ...
        for (var attrname in actionData) {
          data[attrname] = actionData[attrname];
        }
        Debug.log('[[[Command]]]', eventName);
        player.emit(eventName, data);
      };
    }(action.name, action.event, action.data);

    menu.addItem(new MenuItem(action.name, returnFunc));
  }

  menu.on('selected', function(){
    Debug.log("Menu finished selection", menu.name);
  });
