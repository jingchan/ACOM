...
function CreateApplication(){
  var app = {
    ...
    makeMenu: function(player, cursorGridX, cursorGridY) {
      var menu = new Menu(cursorGridX*40, cursorGridY*40, 160, 150);
      menu.player = player;
      ...
      this.listener.simple_combo('space', function(){
        ...
        // self.scene.addEntity(bullet);
          self.menu = self.makeMenu(self.player,
                                    self.cursor.gridPosition.x,
                                    self.cursor.gridPosition.y);
        }
        else if (self.menu) {
