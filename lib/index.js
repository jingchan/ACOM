...
var VisualComponent = require('./component/visual');
...
function CreateApplication(){
  var app = {
    ...
    initialize: function(gameDiv){
      ...
      // the Renderer is the View in MVC. It will also create a Phaser.Game object
    ...
    postRendererCreation: function () {
      ...
      this.agents.push(this.makeAgent('good', r(1,18), r(1,12), 'edgar',
        {animated:true,
         moveSpeed:1,
         acts:[{name:'Chainsaw (r:1)', event:'tryAttack',
                data:{damage:70,
                      range:1,
    ...
    makeAgent: function(team, gx, gy, spriteHandle, opts) {
      ...
      if (0 <= gx && gx <= map.width && 0 <= gy && gy <= map.width) {
        ...
        agent.addComponent(new VisualComponent(spriteHandle, opts));
