  ...
  var app = {
    ...
    postRendererCreation: function () {
      ...
      this.agents.push(this.makeAgent('evil', r(1,18), r(1,12), 'randi',
        {moveSpeed:1,
         acts:[{name:'Vanessa Fist', event:'vanessa',
                data:{damage:50,
                      attackType:'flame'}}]}));
    ...
    makeAgent: function(team, gx, gy, spriteHandle, opts) {
      ...
      if (0 <= gx && gx <= map.width && 0 <= gy && gy <= map.width) {
        ...
        var agent = new Agent(spriteHandle + index.toString(), team, gx, gy);
        // we need health first before rendering...
        agent.addComponent(new HealthComponent(opts.health || 100));
        agent.addComponent(new Visual(spriteHandle, opts));
        this.scene.addEntity(agent);
        agent.addComponent(new MovementComponent({collision: true}));
        var acts = [{name:'DefaultMove', event:'tryMove', data:{}},
                    {name:'Attack', event:'tryAttack', data:{damage:10}}];

        // if (opts.hasOwnProperty('acts')) {
        //   acts = acts.concat(opts.acts);
        // }
        agent.addComponent(new Actor(acts));
        return agent;
      }
      return null;
    },
