...
function CreateApplication(){
  var app = {
    ...
    makeMenu: function(menux, menuy) {
      var menu = new Menu(menux, menuy, 200, 120);
      menu.addItem(new MenuItem('Move', function(){
        Debug.log('Command Chosen', 'move');
      }));
      menu.addItem(new MenuItem('Act', function(){
        Debug.log('Command Chosen', 'act');
      }));
      menu.addItem(new MenuItem('Wait', function(){
        Debug.log('Command Chosen', 'wait');
      }));
      menu.addItem(new MenuItem('Status', function(){
    ...
    _registerKeyboardHandlers: function(){
      ...
      this.listener.simple_combo('up w', function(){
        if (self.menu) {
          self.menu.moveUp();
        }
      });
      this.listener.simple_combo('down s', function(){
        if (self.menu) {
          self.menu.moveDown();
        }
      });
